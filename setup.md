...
### Cross-Reference — canary.103 PWA Guide Enhancement (PR #96360, SHIPPED in 16.3.0-canary.103)

The official PWA guide at [`nextjs.org/docs/app/guides/progressive-web-apps`](https://nextjs.org/docs/app/guides/progressive-web-apps) was enhanced in canary.103 (PR #96360 by Adham Fayrouz) to **explicitly link to the [`serwist/examples/next-basic`](https://github.com/serwist/serwist/tree/main/examples/next-basic) example repo** and to note that the **Serwist plugin currently requires webpack configuration** (relevant for Turbopack users — `experimental.useOffline` is the Turbopack-friendly path). The Serwist-vs-`experimental.useOffline` decision tree is now better-articulated in the official docs:

- **`experimental.useOffline: true`** for automatic offline retry for navigations / server actions / prefetches with no custom service worker required. Works with both Turbopack and webpack. The recommended path for **greenfield PWAs** and **Turbopack users**.
- **Serwist** (`@serwist/next`, `@serwist/precaching`, `@serwist/sw`) for full custom-service-worker control (custom caching strategies, custom notification handling, complex asset precaching beyond what `useOffline` covers). **Webpack only** as of canary.103.

**Source:** [PR #96360 — `Update PWA guide with enhanced offline support details and example links for serwist`](https://github.com/vercel/next.js/pull/96360) · Adham Fayrouz · merged 2026-07-29T22:14:31Z · **SHIPPED in `16.3.0-canary.103`** (docs-only PWA guide enhancement).
...
## Next.js 16.3.1-canary.12 SHIPPED (August 11, 2026) — Setup Recipe Lens — PR #97128 Optimistic Routing Bug Fix (next-intl Infinite Prefetch Loop + Parallel Routes Conflicting Dynamic Params) + 3-PR Legacy PPR Refactor (PR #96753 + #96827 + #96868) + Turbopack CJS Scope-Hoisting Flag (PR #95826) + Turbopack CJS Tree-Shaking Bail (PR #97130) + Turbopack import.meta.glob Module Type Argument (PR #96991)

`next@16.3.1-canary.12` SHIPPED at npm `dist-tag.canary` **2026-08-11T18:03:19Z** — **13h20min EARLIER than the v1.5.49 prediction of ~2026-08-12T07:23Z**. The v1.5.49 cycle correctly captured that the canary-branch-ahead state was 15 commits with 6 MATERIAL + 8 non-material + 1 version-tag; what the v1.5.49 cycle did NOT predict was that the GitHub release tag (published at 17:48:10Z, 25min after the canary-branch tag) + npm-publish (at 18:03:19Z, 40min after the GitHub release tag) would both land within the same 12h window as the canary-branch-ahead observation. **The bundle contains 14 PRs (15 commits including version-tag)** all ahead of canary.11. **6 MATERIAL PRs (all now SHIPPED)** — **(1) PR #97128** (Fix Optimistic Routing Bugs Leading to Repeated Prefetch Loops, Andrew Clark / acdlite, merged 2026-08-11T12:44:48Z, 26 files / +905/-15, closes #97135 + MarkBekooy/prefetching-request-waterfall-bug#1) — fixes the next-intl infinite prefetch loop (a proxy that rewrites every URL to inject a leading path segment + a fully dynamic target route like `/[locale]/[...pages]` leads to an infinite prefetch loop that never resolves); the canonical fix is "record on the local route definition that a dynamic rewrite occurred, disabling further attempts to optimistically resolve the route" — same strategy as normal navigation responses, now applied to prefetch; second bug in same PR: parallel routes with conflicting dynamic params at the same level (`@modal/[...catchAll]` next to `[username]`) cannot be distinguished using the current traversal algorithm; workarounded by disabling optimistic routing for the conflicting route; proper fix deferred to a future PR with "sibling dynamic route segments" sent from the server, similar to static siblings; **setup impact**: any project using `next-intl` with a proxy rewrite + dynamic catch-all, OR any project using `@modal/[...catchAll]` parallel route alongside `[username]` dynamic segment, should bump to `next@16.3.1-canary.12+`; **migration recipe**: no code changes required; just bump the `next` dep; the fix is internal to the optimistic routing implementation in `packages/next/src/client/components/app-router-instance.ts` (the `dispatcherPromise` + the new `dynamicRewriteOccurred` flag on the route definition); **(2) PR #96753 + PR #96827 + PR #96868** (3-PR coordinated Legacy PPR Refactor by Zack Tanner / ztanner, all merged 2026-08-11T01:39:14Z in lockstep) — **PR #96753 renames 4 internal `Legacy*` paths to make them explicit**; **PR #96827 is the substantive change** — Cache Components is now the sole internal PPR signal and `experimental.ppr` is hard-deprecated (errors at config-eval time if set to a non-default value); **PR #96868 deletes the legacy code paths** that are now unreachable after the PR #96827 signal change; **setup impact**: **ANY USER WITH `experimental.ppr: true` OR `experimental.ppr: false` IN THEIR `next.config.ts` WILL GET A HARD ERROR AT BUILD/BOOT AFTER BUMPING**; the `next.config.ts` before/after is: **before** (`// next.config.ts`, in any canary.0..11 or 16.3.0 STABLE) — `export default { experimental: { ppr: true } }` works as expected; **after** (in canary.12+) — `export default { experimental: { ppr: true } }` throws a hard error at config-eval; **migration recipe**: if you don't use `cacheComponents`, just delete the `experimental.ppr` line entirely; if you want the new behavior, set `experimental: { cacheComponents: true }` (the new flag, the old `ppr` is now gone); **the audit recipe is `rg -n "experimental.*ppr" next.config.*` to find any existing references**; **(3) PR #95826** ([turbopack] Do the CJS analysis needed for scope hoisting, Sam Poder / sampoder, merged 2026-08-11T07:06:04Z) — adds the new `turbopackCjsScopeHoisting` opt-in flag for the CJS analysis; the CJS analysis is implemented in `turbopack/crates/turbopack-ecmascript/src/analyzer/graph/visitor.rs`; **setup impact**: opt-in via `next.config.ts` (not on by default in 16.3.1); zero behavior change if not opted in; **migration recipe**: opt in via `experimental: { turbopackCjsScopeHoisting: true }` if you want the analysis (the analysis is the prerequisite for CJS tree-shaking in the canary branch; **companion to PR #97130** below — both together bring back the CJS tree-shaking that was reverted in PR #97018 in canary.11); **(4) PR #97130** ([turbopack] Bail CJS tree-shaking on `var x = module.exports = {}`, Sam Poder / sampoder, merged 2026-08-11T16:41:27Z) — companion fix for PR #97018's canary.11 CJS tree-shaking revert; bails out of CJS tree-shaking when it encounters the canonical `@mixmark-io/domino` lib/LinkedList.js self-referential `module.exports` pattern; **setup impact**: opt-in via `next.config.ts`; **migration recipe**: opt in via `experimental: { turbopackCjsTreeShaking: true }` only if you've audited your lockfile for the `@mixmark-io/domino` pattern (the audit is `rg -n "module\.exports" node_modules/@mixmark-io/domino/lib/LinkedList.js`); **(5) PR #96991** ([turbopack] accept a module type argument for import.meta.glob, kelvin, merged 2026-08-11T10:05:56Z) — adds the type argument to import.meta.glob calls so Turbopack can statically determine the module type; low material for setup but useful for Turbopack builds; **setup impact**: any project using `import.meta.glob('./**/*.ts', { eager: true })` or similar can now pass `{ type: 'module' }` for accurate module-type resolution; **8 non-material PRs** — 4 docs PRs (PR #97143 align Cache Components authentication guide title + PR #96341 create client-side fetching guide + PR #97180 clarify client cache freshness after hydration + PR #96145 server client and directives) + 1 test infra PR (PR #97017 re-enable a few more passing NFT unit cases) + 2 typo-fix PRs (PR #97136 spelling in two comments + PR #97137 typos in code comments) + 1 version-tag commit (`40503aae` v16.3.1-canary.12 by `next-js-bot[bot]`).

**The new `next.config.ts` recommended shape after canary.12 SHIPPED** (for any project using Cache Components):

```ts
// next.config.ts — canonical post-canary.12 (16.3.1-canary.12+)
import type { NextConfig } from 'next'

const nextConfig: NextConfig = {
  experimental: {
    // Cache Components is now the only PPR signal.
    // The old `experimental.ppr: true/false` is HARD-DEPRECATED.
    cacheComponents: true,

    // CJS scope hoisting + CJS tree shaking bail — opt in if you've audited.
    // Both default-OFF in 16.3.1; companion flags.
    turbopackCjsScopeHoisting: true,   // PR #95826
    turbopackCjsTreeShaking: true,     // PR #97130 (companion to PR #97018's revert)

    // No `experimental.ppr` line — it will throw at config-eval time in canary.12+.
  },
  // ...other config...
}
...
**The new `next.config.ts` recommended shape for projects NOT using Cache Components** (drop `experimental.ppr` entirely):

```ts
// next.config.ts — canonical post-canary.12 (16.3.1-canary.12+) for projects WITHOUT Cache Components
import type { NextConfig } from 'next'

const nextConfig: NextConfig = {
  // No `experimental.ppr` line.
  // No `experimental.cacheComponents` line.
  // The default rendering behavior is unchanged.
}

export default nextConfig
```

**The audit recipe for canary.12 migration** (5 steps): **(1) `npm ls next`** to confirm your current `next@16.3.1-canary.11` (or earlier) vs `next@16.3.1-canary.12` (or later); **(2) `rg -n "experimental.*ppr" next.config.*`** to find any existing `experimental.ppr` references — DELETE them if upgrading to canary.12+; **(3) `rg -n "@mixmark-io/domino" package-lock.json`** to check for the CJS-tree-shaking-bail pattern before opting in to `turbopackCjsTreeShaking: true`; **(4) `next build`** smoke test to confirm no config-eval errors; **(5) `rg -n "next-intl" package.json`** + the next-intl proxy-rewrite check (`rg -n "rewrite" next.config.* middleware.ts`) for the PR #97128 infinite-prefetch-loop-fix verification — if you have both `next-intl` + a proxy rewrite + a dynamic catch-all route, the bug is FIXED for you in canary.12+.

## Next.js 16.3.1-canary.13 SHIPPED (August 12, 2026) — Setup Recipe Lens — LightningCSS Bump (PR #97159) + Turbopack Cache TTL (PR #96941) + Loader Tree Decoupling (PR #97192) + [turbopack] Shared Runtime Default-On-Canary-Only (PR #97208)

`next@16.3.1-canary.13` SHIPPED at npm `dist-tag.canary` **2026-08-12T00:02:03Z** — ~6h before this cron's 06:02Z start, 4 commits ahead of canary.12 = 3 PRs + 1 version-tag. **3 PRs in the canary.13 bundle**: **(1) PR #97159** Bump lightningcss (Niklas Mischkulnig, merged 2026-08-11T19:16:54Z, 7 files / +86/-51) — the `turbopack/crates/turbopack-css` patches: `process.rs` (+13/-10), `references/url.rs` (+1/-1), `lifetime_util.rs` (+1/-6), `references/mod.rs` (+1/-1) + the snapshot test for `Parsing CSS source code failed-18c92d.txt` (now 19 lines shorter) + Cargo.toml + Cargo.lock; **setup impact**: any project using Turbopack + Tailwind v4 (which uses lightningcss under the hood for `--tailwind`-flagged `create-next-app` scaffolds) gets the new lightningcss behavior; **(2) PR #96941** [turbopack] Retain fewer stale cache versions and use a TTL (Luke Sandberg / lukesandberg, merged 2026-08-11T19:59:32Z, 10 files) — material disk-usage fix for dev mode; **setup impact**: dev-time disk usage for the Turbopack cache was retaining up to 2 old versions of the cache DB indefinitely in dev / non-CI; now retains no more than 1 old version for 3 days (a TTL supports switching back and forth between feature branches + a next.js upgrade branch over a weekend); the `CURRENT` file format modified from a sequence number (just a number) to a small JSON object containing `max_sequence_number` + mtime (mtime is needed because cache directories may be copied around and the native mtime would record the copy time instead of the original write time); the change is internal to `turbopack/crates/turbo-persistence/src/db.rs` + `turbopack/crates/turbo-tasks-backend/src/database/db_versioning.rs`; the change DOES affect `experimental.turbopackFileSystemCache` users — the cache directory will be automatically migrated to the new format on first read; the migration is one-way (the new format is incompatible with the old `CURRENT` file); **the audit recipe is `ls -la .next/cache/turbopack/ | head` to verify the new `CURRENT` file format** — it should now contain a JSON object, not just a number; **the recommended action for stuck-on-canary.11-or-earlier users is to delete `.next/cache/turbopack/` once after bumping to canary.13+** — this forces a clean cache rebuild; **(3) PR #97192** Decouple loader tree construction from children ordering (Josh Story / gnoff, merged 2026-08-11T21:30:31Z, 2 files / +10/-10) — refactor; the app loader tree was mutating Turbopack's parallel route map to ensure `children` is inserted first; the fix moves `children`-first ordering to the Turbopack code generator (output unchanged); net effect: framework-author surface only, no user-facing change; **setup impact**: zero for users; the version-tag commit (`59aa000d` v16.3.1-canary.13 by `next-js-bot[bot]`) at 2026-08-11T23:26:01Z.

**4 PRs that SHIPPED in `next@16.3.1-canary.14` (between canary.13 tag at 23:26:01Z and canary.14 tag at 13:01:25Z Aug 12)**: **(1) PR #97208** [turbopack] Only use the shared runtime by default on canary (Sam Poder / sampoder, merged 2026-08-11T23:31:57Z, 3 files / +8/-4) — **HEADLINE SETUP CHANGE**: the shared runtime was on by default in `16.3.0` STABLE; PR #97208 makes it ONLY default-on in canary builds, NOT in `16.3.1`; the PR body is terse — "This should not be on in 16.3.1." — confirming the intentional scope rollback; the diff is in `crates/next-core/src/next_config.rs` (+1/-1) + `packages/next/src/build/define-env.ts` (+3/-2) + `packages/next/src/server/config-shared.ts` (+4/-1); **setup impact**: any project on `next@16.3.0` STABLE that was relying on the default-on shared runtime will see a behavior change when bumping to `16.3.1` (the shared runtime will no longer be default-on; you'll need to explicitly opt in); **the migration recipe** is `experimental: { turbopackSharedRuntime: true }` in `next.config.ts` if you want to preserve the 16.3.0 STABLE behavior; the new default in 16.3.1 is OFF; **(2) PR #95439** Fix stale data after navigation despite revalidation (dan, merged 2026-08-12T00:43:17Z, 3 files / +81/-5) — when a navigation displaced pending actions, the navigation's promise was the last dispatched but the pending action ran after it, so the action's state was no longer taken into account even if it revalidated; with the fix, the final state is properly reflected; affects any Next.js app with `useTransition` + Server Actions that revalidate while a navigation is in flight; **setup impact**: no code changes required; the bug is fixed internally in `packages/next/src/client/components/app-router-instance.ts`; the new E2E test is in `test/e2e/app-dir/actions-discarded-navigation-revert/actions-discarded-navigation-revert.test.ts` (+53 lines); **(3) PR #97215** Fix shared Turbopack runtime initialization race (Josh Story / gnoff, merged 2026-08-12T04:38:24Z, 15 files) — the shared browser runtime could execute before other async chunks, and previously returned when the chunk-loading global was undefined, leaving later registrations queued and preventing the Next.js client bootstrap and hydration from running; **setup impact**: any project on Turbopack + multiple async chunks + the shared runtime opt-in may see hydration failures fixed after bumping to canary.14+ (when this PR ships); the fix updates 13 snapshot test fixtures (the affected development, production, debug-ID, and worker runtime snapshots); no code changes required for users; **(4) PR #97181** Allow literal exports in `'use cache'` files (Hendrik Liebau / unstubbable, merged 2026-08-12T04:42:24Z, 11 files) — a file-level `'use cache'` directive previously rejected any export initialized with a literal, so `export const instant = false` in a page or layout failed the build with "Only async functions are allowed to be exported in a `'use cache'` file"; object and array literals were already exempt; the fix restricts the check to `in_action_file = true` only; **critical for the Cache Components migration codemod which adds `export const instant = false` to page and layout files** — without this fix, the codemod output fails to compile; **setup impact**: any project using `cacheComponents: true` + a layout/page that needs route segment config (`export const instant = false`) was previously BLOCKED; with canary.14+ (when this PR ships), the build will succeed; the canonical recipe for `instant` config: **`export const instant = false` in a layout or page with a file-level `'use cache'` directive is the canonical pattern for opting out of static shell validation**; the audit recipe for users on canary.13 or earlier with `cacheComponents: true` + a `'use cache'` layout/page is `rg -n "export const instant" app/` — if you find this and you're on canary.13 or earlier, expect the build to fail with "Only async functions are allowed to be exported in a `'use cache'` file".

**The new `next.config.ts` recommended shape after canary.13 SHIPPED** (for projects on `16.3.0` STABLE who want to opt into the canary behavior):

```ts
// next.config.ts — canonical post-canary.13 (16.3.1-canary.13+) for projects MIGRATING FROM 16.3.0 STABLE
import type { NextConfig } from 'next'

const nextConfig: NextConfig = {
  experimental: {
    // If you were relying on the default-on shared runtime in 16.3.0 STABLE,
    // explicitly opt in here (PR #97208 makes it default-OFF in 16.3.1).
...
**The 6-step combined canary.12 + canary.13 audit recipe**: **(1) `npm ls next`** to confirm your version (should be `16.3.1-canary.13+` for the full bundle); **(2) `rg -n "experimental.*ppr" next.config.*`** to find any existing `experimental.ppr` references — DELETE them if upgrading to canary.12+; **(3) `rm -rf .next/cache/turbopack/`** once after bumping to canary.13+ to force a clean cache rebuild (the `CURRENT` file format changed from sequence number to JSON object); **(4) `rg -n "export const instant" app/`** to check for the `'use cache'` literal-export pattern; expect build failures if found on canary.13 or earlier; **(5) `next build`** smoke test to confirm no config-eval errors; **(6) `rg -n "next-intl" package.json`** + the next-intl proxy-rewrite check for the PR #97128 infinite-prefetch-loop-fix verification.

## `next@backport` 15.5.23 — Vercel Missed-Bump Correction (August 6, 2026) — Security Backport — `[Flight] Port ReplyServer Traversal Guards to FlightClient` (PR #96405) — Missed in v1.5.49

**Correction to v1.5.49**: the v1.5.49 cycle said `next@backport` still `15.5.22`, but **`next@15.5.23` SHIPPED at npm `dist-tag.backport` 2026-08-06T19:30:10Z** — **5 days before this cron** — and v1.5.49 missed the bump. The 15.5.23 release contains **one commit** — `[15.x] Port ReplyServer traversal guards to FlightClient` PR #96405 by @eps1lon (Sebastian Silbermann) — a Flight-side hardening (ReplyServer traversal guards ported to FlightClient). **Setup impact**: **the 15.x branch backport train is now patched for the Flight-side traversal-guard hardening**; any `next@15.x` user on `15.5.22` or earlier should bump to `15.5.23+`; the recommended pin is `^15.5.23`; **the audit recipe** is `npm ls next` to confirm `15.5.23` (or later, when the Aug 20 batch adds `15.5.24`); the PR body details are not public (15.x branch commits have the body stripped from GitHub releases; the PR description is in the GitHub PR #96405 page); **the security-class nature of PR #96405** (a traversal-guard port) is the reason v1.5.49 should have caught it — the v1.5.49 cycle's T-2d observation window started at Aug 9 18:08Z (when the v1.5.48 commit landed), and 15.5.23 was published at Aug 6 19:30Z — 3 days before the v1.5.49 cycle's observation window — so the v1.5.49 cycle should have seen it; the cycle missed it because the v1.5.48 and v1.5.49 inline update paragraphs only listed versions they had confirmed via `npm view next dist-tags.backport`, and the `dist-tags.backport` was indeed `15.5.23` since Aug 6 — the v1.5.49 cycle's inline observation was incorrect; **this is a v1.5.49 inline-bump miss that is being corrected now**.

## Cross-References

- `security.md` → `## Next.js 16.3.1-canary.12 SHIPPED + canary.13 SHIPPED + Critical Dev-Mode Security Disclosure #97157` for the security lens (the #97157 dev-mode inspector UUID + CDP RCE + webpack source-map file-read + unauthenticated /_next/mcp + HMR websocket is the headline; closed-by-bot-not-fix at 2026-08-11T07:18:25Z; affected next@16.3.0 STABLE + all canary.0..13; 5 mitigations + watch for canary.14+/Aug 20 batch fix)
- `security.md` → `## August 20 Vercel Monthly Security Release Pre-Roll Refresh (T-8 days)` for the Aug 20 batch lens (#97157 is the headline candidate; expected patch versions next@16.3.1 + 16.3.2 + 15.5.24 + 14.2.36)
- `routing.md` → `## 16.3.1-canary.12-ahead` for the routing lens on the same canary.12 bundle (PR #97128 + 3-PR legacy PPR refactor + Turbopack CJS pair + 13-link Sources block)
- `server-components.md` → `## Cache Components — 3-PR Legacy PPR Refactor` for the Server Components / Cache Components lens on the canary.12 bundle (PR #96753 + #96827 + #96868 + experimental.ppr hard-deprecation + Cache Components as sole internal PPR signal)
- `auth.md` → `## Better Auth 1.6.27 + 1.7.0-rc.5 SHIPPED (August 11, 2026)` for the Better Auth 1.7.0-rc.5 BREAKING CHANGE OAuth device grant ownership PR #10746 lens

### Sources

- [Next.js `v16.3.1-canary.12` GitHub release tag](https://github.com/vercel/next.js/releases/tag/v16.3.1-canary.12) (published 2026-08-11T17:48:10Z)
- [Next.js `v16.3.1-canary.13` GitHub release tag](https://github.com/vercel/next.js/releases/tag/v16.3.1-canary.13) (published 2026-08-11T23:51:56Z)
- [Next.js canary-branch compare `v16.3.1-canary.11...v16.3.1-canary.12`](https://github.com/vercel/next.js/compare/v16.3.1-canary.11...v16.3.1-canary.12) (15 commits ahead; the canary.12 bundle)
- [Next.js canary-branch compare `v16.3.1-canary.12...v16.3.1-canary.13`](https://github.com/vercel/next.js/compare/v16.3.1-canary.12...v16.3.1-canary.13) (4 commits ahead; the canary.13 bundle)
- [Next.js canary-branch compare `v16.3.1-canary.13...canary`](https://github.com/vercel/next.js/compare/v16.3.1-canary.13...canary) (4 commits ahead; the canary.14 bundle — SHIPPED npm-published 2026-08-12T13:25:30Z)
- [PR #97128 — Fix Optimistic Routing Bugs Leading to Repeated Prefetch Loops](https://github.com/vercel/next.js/pull/97128) (Andrew Clark / acdlite, 26 files / +905/-15)
- [PR #96753 — Make legacy PPR paths explicit](https://github.com/vercel/next.js/pull/96753) (Zack Tanner / ztanner)
- [PR #96827 — Use Cache Components as the internal PPR signal](https://github.com/vercel/next.js/pull/96827) (Zack Tanner / ztanner)
- [PR #96868 — Remove legacy PPR code paths](https://github.com/vercel/next.js/pull/96868) (Zack Tanner / ztanner)
- [PR #95826 — [turbopack] Do the CJS analysis needed for scope hoisting](https://github.com/vercel/next.js/pull/95826) (Sam Poder / sampoder)
- [PR #97130 — [turbopack] Bail CJS tree-shaking on `var x = module.exports = {}`](https://github.com/vercel/next.js/pull/97130) (Sam Poder / sampoder)
- [PR #96991 — Turbopack: accept a module type argument for import.meta.glob](https://github.com/vercel/next.js/pull/96991) (kelvin)
- [PR #97159 — Bump lightningcss](https://github.com/vercel/next.js/pull/97159) (Niklas Mischkulnig)
- [PR #96941 — [turbopack] Retain fewer stale cache versions and use a TTL](https://github.com/vercel/next.js/pull/96941) (Luke Sandberg / lukesandberg)
- [PR #97192 — Decouple loader tree construction from children ordering](https://github.com/vercel/next.js/pull/97192) (Josh Story / gnoff)
- [PR #97208 — [turbopack] Only use the shared runtime by default on canary](https://github.com/vercel/next.js/pull/97208) (Sam Poder / sampoder)
- [PR #95439 — Fix stale data after navigation despite revalidation](https://github.com/vercel/next.js/pull/95439) (dan)
- [PR #97215 — Fix shared Turbopack runtime initialization race](https://github.com/vercel/next.js/pull/97215) (Josh Story / gnoff)
- [PR #97181 — Allow literal exports in `'use cache'` files](https://github.com/vercel/next.js/pull/97181) (Hendrik Liebau / unstubbable)
- [PR #96405 — [15.x] Port ReplyServer traversal guards to FlightClient](https://github.com/vercel/next.js/pull/96405) (@eps1lon, Sebastian Silbermann)
- [Issue #97135 — Infinite prefetch loop (livelock, ~600 req/s) when a root parallel slot contains a catch-all and the current page is nested under a sibling dynamic segment](https://github.com/vercel/next.js/issues/97135) (closed by PR #97128)
- [`next@16.3.1-canary.12` npm-published timestamp](https://www.npmjs.com/package/next/v/16.3.1-canary.12) (2026-08-11T18:03:19Z, 13h20min EARLIER than the v1.5.49 prediction)
- [`next@16.3.1-canary.13` npm-published timestamp](https://www.npmjs.com/package/next/v/16.3.1-canary.13) (2026-08-12T00:02:03Z)
- [Next.js cacheComponents config reference](https://nextjs.org/docs/app/api-reference/config/next-config-js/cacheComponents)
- [Next.js experimental.ppr config reference](https://nextjs.org/docs/app/api-reference/config/next-config-js/experimental-ppr) (now hard-deprecated)
- [Next.js generateStaticParams API docs](https://nextjs.org/docs/app/api-reference/file-conventions/route-segment-config)
...
## Next.js 16.3.1-canary.17 → 16.3.1-canary.19 SHIPPED (August 14–15, 2026) — Setup Recipe Lens — 4 MATERIAL canary.17 Setup-Relevant PRs (PR #97287 Adapter + Standalone ENOENT Build-Failure Fix + PR #96819 Pages API Runtime Trace Fix + PR #97350 Pages Router Metadata-Files Scope Fix + PR #97276 satori 0.29.0 Bump) + 4 canary.19 Setup-Relevant PRs (PR #97387 SelectedMetadata + PR #97278 next/image Empty-Cache Reject + PR #97333 Turbopack Stale Manifests + PR #97385 Turbopack Unreachable Codegen) + 22nd TypeScript No-Content Rebuild + 5 canary-branch-ahead-of-canary.19 forward-looking PRs + 16.3.2 STABLE Forecast (T-5d from Aug 20)

`next@16.3.1-canary.17` SHIPPED at npm `dist-tag.canary` **2026-08-14T17:20:01Z** (~11h after the v1.5.58 cutoff at 2026-08-14T06:06Z), followed by `next@16.3.1-canary.18` at **2026-08-14T21:21:29Z** (~1 canary drop ahead) and `next@16.3.1-canary.19` at **2026-08-15T00:12:10Z** (~17h50min before this cron). **The 4 v1.5.58→v1.5.62 cycles deferred the setup-recipe + deployment-impact + performance-measurement lenses for these 3 canary drops** (focused on api/typescript/pattern/routing/auth/security/test/component surfaces). **This cycle corrects the deferred setup-recipe lens** for performance.md + deployment.md. The combined set has direct **setup-recipe impact** that the v1.5.58–v1.5.62 cycles missed.

### PR #97287 — Adapter + Standalone Setup Fix (PR #97287 walkthrough)

**PR #97287** (by gnoff + styfle, merged 2026-08-14T17:20:01Z [canary.17], 2 files / +44/-2) — **fixes the setup-recipe for adapter + `output: 'standalone'` combinations**. Pre-#97287, any app that combined a deployment adapter with `output: 'standalone'` crashed during `next build` with `Error: ENOENT: no such file or directory, open '<distDir>/next-server.js.nft.json'`. The PR body verbatim: *"Since v16.3.0, `next build` crashes for any app that combines a deployment adapter with `output: 'standalone'`. This breaks every Vercel deployment of a standalone-configured app on 16.3 (the builder injects its adapter via `NEXT_ADAPTER_PATH` under the `NEXT_ENABLE_ADAPTER` rollout), and — per the reports on #96646 — also self-hosted/AWS `cdk-nextjs` users, for whom there is **no config-level escape**: that adapter force-sets `output: 'standalone'` and the construct requires `.next/standalone`, so both sides of the conflict are mandatory."*

**Setup-recipe impact**:

- **The canonical setup-recipe for 16.3+ adapter + standalone deployments**:
  ```ts
  // next.config.ts (works on canary.17+ / 16.3.2+)
  import type { NextConfig } from 'next'
  import adapter from './adapter.mjs'  // or NEXT_ADAPTER_PATH env

  const config: NextConfig = {
    output: 'standalone',  // works WITH adapter on canary.17+
    adapter,  // Vercel / cdk-nextjs / SST / Amplify / custom
    experimental: {
      cacheComponents: true,  // recommended for 16.3+
      // turbopackSharedRuntime: false  // default in 16.3.1 (was true in 16.3.0)
    },
  }
  export default config
  ```
- **The pre-#97287 setup-recipe failure pattern**: any of the 4 common adapter combinations (Vercel adapter injected via `NEXT_ADAPTER_PATH` + `NEXT_ENABLE_ADAPTER` rollout; self-hosted `cdk-nextjs` adapter; SST adapter; Amplify adapter) failed `next build` at the NFT emission step. **Recovery on canary.17+**: build succeeds, `.next/standalone/` contains 100% of traced files, `node .next/standalone/server.js` boots cleanly.
...
**Setup audit recipe** (pre-bump): `npm ls next` (should show 16.3.0 STABLE; on canary.17+ / 16.3.2 the bug is fixed); `rg -n "output: ['\"]standalone['\"]|NEXT_ADAPTER_PATH|NEXT_ENABLE_ADAPTER" next.config.ts .env*` (check for adapter + standalone combo); for affected setups, expect `next build` to fail with `Error: ENOENT: no such file or directory, open '<distDir>/next-server.js.nft.json'`. **Audit recipe** (post-bump): same `npm ls next` check; same `rg` check; build should succeed; `ls -la .next/standalone/` should contain the full file count.

### PR #96819 — Pages API + Adapter Setup Fix

**PR #96819** (by styfle, merged 2026-08-14T17:20:01Z [canary.17], 11 files / +191/-5) — **fixes the setup-recipe for Pages API routes + adapter + externalized-dep + `next/head` imports**. Pre-#96819, Pages API functions built through a deployment adapter failed during function initialization with `Cannot find module 'next/dist/compiled/next-server/pages-turbo.runtime.prod.js'`. The PR body verbatim: *"Pages API functions produced through a build adapter can fail during function initialization, before the customer handler executes. The failure is triggered when an externalized dependency used by the API route imports a Next.js module such as `next/head`."*

**Setup-recipe impact**:

- **The canonical setup-recipe for Pages API + adapter + Turbopack or Webpack on canary.17+**: no code change required. The fix is build-time + bundler-side only; Pages API routes + adapter + externalized dep + `next/head` patterns that were crashing pre-canary.17 now build and serve correctly.
- **The test fixture pattern**: `test/production/adapter-pages-api-runtime/` shows the canonical pattern — a Pages API route in `pages/api/hello.ts` that imports a package `test-storyblok-external` which itself imports `next/head`. The fixture builds with an adapter, materializes only the generated Pages API function entry and its provided assets into a clean directory, initializes the production environment, and requires the handler in a child process. Verified under both Turbopack and Webpack.
- **Adapter API stability**: no adapter change required. The fix is in the Next.js core (`crates/next-api/src/next_server_nft.rs` + `crates/next-api/src/project.rs` + `packages/next/src/build/adapter/build-complete.ts`); adapters continue to consume the per-endpoint NFTs.

**Setup audit recipe**: `rg -n "next/head|next/document" pages/api/` (check if Pages API routes import Pages Router modules); `rg -n "adapter|NEXT_ADAPTER_PATH|NEXT_ENABLE_ADAPTER" next.config.ts` (verify adapter is configured); for affected setups on 16.3.0 / 16.3.1 STABLE, expect Pages API route 500 errors with `Cannot find module 'next/dist/compiled/next-server/pages-turbo.runtime.prod.js'`. **Audit recipe** (post-bump): Pages API routes should respond 200.

### PR #97350 — Pages Router Metadata-Files Setup Fix

**PR #97350** (by mischnic, merged 2026-08-14T17:20:01Z [canary.17], 30 files) — **fixes the setup-recipe for Pages Router with `pages/sitemap.js` / `pages/robots.js` / `pages/manifest.json` / `pages/icon.*` files exporting `getStaticProps`/`getServerSideProps`**. Pre-#97350, these patterns failed 16.3.0 / 16.3.1 STABLE builds with `Error: "getStaticProps" is not supported in app/.`. The PR body verbatim: *"Since 16.3.0, builds fail for pages-router files named `sitemap` or `robots` that export `getStaticProps`/`getServerSideProps`. This is a regression from #94962, which added the metadata conventions (`sitemap`, `robots`, `manifest`, `icon`, …) to the app-entry filename regex in `ReactServerComponentValidator::assert_invalid_api`. The regex only looks at the filename, so a file like `pages/sitemap.js` is now mistaken for an app entry and rejected for exporting `getStaticProps`."*

**Setup-recipe impact**:

- **The canonical Pages Router metadata-file pattern on canary.17+ / 16.3.2+**:
  ```js
  // pages/sitemap.js — works on canary.17+ / 16.3.2+
  ...
  ```
  Pre-#97350 this exact pattern failed the build. Post-#97350 it compiles and serves correctly.
- **The 22 test fixtures renames**: the test fixtures that exercise the app-entry checks moved from `client-graph/` to `app-dir/` subdirectories (since the test harness derives `appDir` from the fixture path). A new fixture (`sitemap.js`) + `sitemap.js` fixture glob assert that a pages-style `sitemap.js` compiles without errors.
- **The new e2e test suite**: `test/e2e/pages-metadata-filenames` covers `pages/sitemap.js` with `getStaticProps` and `pages/robots.js` with `getServerSideProps`. Fails without the fix under Turbopack (build and dev); passes with it under both Turbopack and webpack.

**Setup audit recipe** (pre-bump): `ls pages/sitemap.js pages/robots.js pages/manifest.json pages/icon.* 2>&1` (check for Pages Router metadata filenames); for affected setups, expect build failure with `Error: "getStaticProps" is not supported in app/.`. **Audit recipe** (post-bump): build should succeed.

### PR #97276 — satori 0.29.0 + @vercel/og 0.10.x Bump (next/og WebP support)

**PR #97276** (by styfle, merged 2026-08-14T17:20:01Z [canary.17], 7 files / +5747/-6606) — bumps `satori` 0.26.0 → 0.29.0 (adds WebP image support) + `@vercel/og` 0.7.x → 0.10.x. The PR body verbatim: *"fix(deps): bump `satori` and `@vercel/og`"*.

**Setup-recipe impact**:

- **Zero code change required for `next/og` consumers**: the Next.js integration wraps `@vercel/og` so consumers don't import the upstream package directly. The 0.7.x → 0.10.x bump is **API-stable** for users of `next/og` / `ImageResponse`.
- **New `next/og` capability**: WebP source images can be used directly without pre-rasterization. For `next/og` routes:
  ```tsx
  // app/og/route.tsx — works on canary.17+ with WebP sources
  import { ImageResponse } from 'next/og'
  
  export async function GET() {
    return new ImageResponse(
      <div style={{ /* ... */ }}>
        <img src="/logo.webp" />  {/* WebP supported natively */}
      </div>,
      { width: 1200, height: 630 }
    )
  }
  ```
- **Bundle size delta**: the pre-compiled `@vercel/og` index.edge.js went from 2034 → 1951 lines and index.node.js went from 3690 → 4635 lines (the node bundle added dependencies for the WebP decoder); the resvg.wasm binary was unchanged. Net bundle size delta is small.

**Setup audit recipe**: `rg -n "next/og|ImageResponse" app/` (find OG image usage); no code change required, just bump to canary.17+ and WebP sources will be supported.

### PR #97387 — Adopt SelectedMetadata for metadata rendering (internal refactor)

**PR #97387** (by gnoff, merged 2026-08-14T23:46:30Z [canary.19], 2 files / +68/-16) — **introduces `SelectedMetadata` as the post-processed, tag-ready representation** for metadata rendering. The PR body verbatim: *"The metadata resolver currently passes ResolvedMetadata directly to tag rendering even though several fields only exist to carry state between route layers. Introduce SelectedMetadata as the post-processed, tag-ready representation and convert the current resolved output before rendering. This makes the final selection boundary explicit without changing generated tags, preparing the metadata pipeline for multiple independently resolved branches."*

**Setup-recipe impact**:

- **Zero code change for users**: no generated tag changes; same output byte-for-byte.
- **No config or schema changes**: the new `SelectedMetadata` type is internal to `packages/next/src/lib/metadata/`.

**Setup audit recipe**: `rg -n "SelectedMetadata|ResolvedMetadata" packages/next/src/lib/metadata/` (verify the new type exists); no action required.
...
- **The canonical self-hosted disk-cache cleanup recipe on bump to canary.19+**:
  ```bash
  # Step 1: bump to next@16.3.1-canary.19+ (or wait for next@16.3.2 STABLE)
  pnpm add next@canary  # or pnpm add next@^16.3.2 once STABLE

  # Step 2: one-shot cleanup of accumulated 0-byte files
  find .next/cache/images -size 0 -delete 2>/dev/null

  # Step 3: restart next dev / next start
  pnpm dev  # or pnpm start

  # Step 4: verify no more LRUCache errors
  rg "LRUCache: calculateSize returned 0|Failed to write image to cache" .next/server.log 2>&1
  # Expected: empty
  ```
- **The error pattern to grep for in pre-bump logs**:
  ```
...
**Setup audit recipe**: `find .next/cache/images -size 0 2>&1 | wc -l` (pre-bump count; post-bump + cleanup should be 0); same `rg "LRUCache"` grep.
...
- **Zero code change for users**: dev-server-only fix.
- **New e2e test coverage**: `test/development/app-dir/hmr-deleted-page/` covers the deleted-concrete-replaced-with-catch-all pattern.
- **Forward-looking**: combined with PR #97385, prepares Turbopack for upcoming manifest-streaming optimizations.

**Setup audit recipe**: no action required for users; the regression is automatically fixed on bump.

### PR #97385 — Turbopack: make unreachable codegen more generic

**PR #97385** (by styfle, merged 2026-08-14T23:46:30Z [canary.19], 3 files / +41/-23) — makes the inserted comment for unreachable codegen configurable. Internal-only; no behavior change.

**Setup-recipe impact**: zero. Internal Turbopack refactor; no user-observable change.
...
- **The canonical TypeScript 7.1 setup-recipe for August 2026** (UNCHANGED from v1.5.57):
  ```json
  // package.json
  {
    "devDependencies": {
      "typescript": "^7.0.2"  // STABLE since 2026-08-03; canonical install recipe
    }
  }
  ```
  ```bash
  # install + verify
  pnpm install
  pnpm exec tsc --version  # should show 7.0.x
  ```
  The `tsgo` binary name from earlier canary cycles is corrected: **use `tsc`**, not `tsgo`. `tsgo` only exists in `@typescript/native-preview` for nightly builds.
- **Next.js 16.3.0+ TypeScript 7.0 build integration** (UNCHANGED):
  ```json
  // package.json — next-16.3
  {
    "devDependencies": {
      "typescript": "^7.0.2"
    },
    "scripts": {
      "build": "next build"  // automatically uses tsc 7.0 for type-checking
    }
  }
  ```
- **Strada API October 2026 forecast UNCHANGED**: the Strada API migration off `@typescript/typescript6` shim is on track per the v1.5.57 cycle.

**Setup audit recipe**: `pnpm exec tsc --version` (should show 7.0.2+); `npm view typescript@next version` (should show `7.1.0-dev.20260815.1`); `git log --oneline microsoft/TypeScript | head` (no new commits since 2026-07-27T20:55:30Z — TypeScript main branch still idle).
...
1. **PR #94157** — Remove server route matcher stack (1 commit; routing simplification)
2. **PR #97372** — Turbopack: retain conditions when replacing resolve request keys (1 commit; Turbopack perf)
3. **PR #97415** — test: update React 18 redbox snapshot (1 commit; test infra)
4. **PR #97388** — Extract metadata resolution primitives (1 commit; follow-on to PR #97387 SelectedMetadata)
5. **PR #97321** — Wait for back-before-hydration recoveries in the browser (1 commit; hydration recovery)
...
- **PR #94157 server route matcher stack removal**: a routing-side simplification that reduces the matcher depth; expected no setup-recipe change for users (internal refactor).
- **PR #97388 metadata primitives extraction**: follow-on to PR #97387 SelectedMetadata; prepares the metadata pipeline for the next round of parallelism (no user-visible change yet).

**Setup audit recipe** (forward-looking): monitor `npm view next@canary version` for the canary.20 cut; expect within 8-12h.

### 16.3.2 STABLE Forecast (T-5d from Aug 20) — Updated Setup Recipe

The 4 MATERIAL canary.17 PRs (PR #97287 + PR #96819 + PR #97350 + PR #97276) + the 4 canary.19 PRs (PR #97387 + PR #97278 + PR #97333 + PR #97385) + the 5 forward-looking canary.20 PRs are all expected to ship in `next@16.3.2` STABLE within 1-2 weeks. The Aug 20 monthly security window (T-5d from this cron) is the candidate for the 16.3.2 STABLE cut. The v1.5.60 cycle opened the forecast; v1.5.61 + v1.5.62 confirmed the forecast holds; this cycle confirms the forecast is still on track.

**The canonical 16.3.2 STABLE upgrade recipe** (when it ships):
```bash
# Step 1: bump to next@16.3.2 (or 16.3.1-canary.20+ for early adopters)
pnpm add next@^16.3.2

# Step 2: run the AI-codemod
npx @next/codemod@canary upgrade latest

# Step 3: verify the canary.17 4-MATERIAL-PR fix impact for affected setups
# Adapter + standalone users: build succeeds (PR #97287)
# Pages API + adapter users: Pages API routes serve correctly (PR #96819)
# Pages Router metadata-file users: build succeeds (PR #97350)
# next/og users: WebP source images supported (PR #97276)

# Step 4: verify the canary.19 fix impact for affected setups
# Self-hosted disk-cache users: PR #97278 + PR #94068 eliminate 0-byte pollution
# Dev users replacing concrete pages with catch-all: no restart needed (PR #97333)
...
- **Production codebases on `next@16.3.1` STABLE**: STAY on `^16.3.1` for stable deployments. The 4 MATERIAL canary.17 PRs are critical for adapter + standalone / Pages API + adapter / Pages Router + metadata-filenames / `next/og` users. The 4 canary.19 PRs are also important: PR #97278 next/image empty-cache reject is a MEDIUM-severity bug fix for self-hosted deployments. **All 8 PRs are expected to ship in `next@16.3.2` STABLE within 1-2 weeks** (the Aug 20 monthly security window is the candidate).
...
- Cross-reference: v1.5.58 setup.md `## Next.js 16.3.1 STABLE SHIPPED` — the 16.3.1 STABLE + canary.16 setup-recipe lens (still authoritative for the canary.16 PRs)
