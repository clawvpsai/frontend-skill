...
3. **For teams still on stable `next@16.2.x`:** nothing changes — the vendor React bump only applies to `next@canary`. Stable still ships with React `19.2.7` bundled. To pick up PR #37030 + #37039 on stable, you must wait for either (a) the July 20, 2026 security release (which may bump `react` for security-adjacent fixes if Vercel decides to include the nonce-hydration fix — historical precedent from the May 2026 release bundled React security fixes; verdict uncertain for July 20), or (b) React 19.3 stable (no public date yet — React's release cadence has been ~6 months since 19.2 shipped October 2025, so a 19.3 stable in Q1 or Q2 2026 is plausible).
