## canary.12 + zod@4.5.3 + zod@4.5.4 SHIPPED (Aug 29) + TypeScript 7.0 STABLE + Aug CVE Retrospective (Setup Lens — v1.6.15, August 30, 2026)

**This cycle refreshes the stale v1.5.94 setup.md header (Aug 24 "Aug 26 CVE T-2d") with current state, adds canary.12, documents zod@4.5.3 + 4.5.4 setup implications, and fills the critical gap: TypeScript 7.0 STABLE shipped July 8 but has been missing from all skill docs.**

### Aug CVE Retrospective — Shipped Aug 25 as next@16.3.3 + next@15.5.24

The Aug 26 security release was **moved up to Aug 25** (announced Aug 20). Two critical CVEs:
- **CVE-2026-75604**: AVIF image processing RCE. Vercel disabled AVIF immediately. **Fixed in next@16.3.3 + next@15.5.24** — AVIF served as-is until libheif fix.
- **Second critical CVE**: Windows unauthenticated RCE. **No workaround; upgrade immediately.**

**Setup implication**: The AVIF fix means `next/image` AVIF auto-conversion is disabled. Apps that use `next/image` with AVIF sources should test after upgrade. The Windows CVE only affects Windows-hosted Next.js servers.

### next@16.4.0-canary.12 SHIPPED (npm-published 2026-08-29T23:38:15Z)

**canary.12 is a minimal release (5 PRs)**. No breaking changes for setup. The PR #98029 (`.mjs codemod transform`) may affect projects that use `.mjs` module files — run `next lint` after upgrade to check.

**Setup recipe**:
```bash
# Upgrade to canary.12
npm install next@canary

# Verify version
npm ls next
# Expected: 16.4.0-canary.12 or later

# Test .mjs codemod compatibility
npm run lint
```

### zod@4.5.3 + zod@4.5.4 SHIPPED Aug 29 (npm-published 2026-08-29T17:51Z + 17:55Z)

**Two PATCH releases within 4 minutes** — missed by v1.6.14 (which tracked zod@4.5.2). These are the 3rd + 4th PATCHes since zod@4.5.0 (Aug 28).

- **zod@4.5.3** (17:51Z): pure PATCH — bug fixes. Check [releases page](https://github.com/colinhacks/zod/releases).
- **zod@4.5.4** (17:55Z): **Latest STABLE**. Also pure PATCH.

**Setup implication**: The zod@4.5.1 breaking changes (datetime seconds requirement + CUID v1 deprecation + base64 whitespace rejection) are still the highest-impact items for setup audits. The 4.5.3/4.5.4 PATCHes are bug fixes on top.
...
# Audit for zod 4.5.1 breaking changes (if not already done):
# 1. datetime audit — z.iso.datetime() now requires seconds
rg "iso\.datetime\|\.datetime\(\)" --type ts | grep -v node_modules | head -20

# 2. CUID v1 deprecation — z.string().cuid() rejects CUID v1
rg "\.cuid\(\)" --type ts | grep -v node_modules | head -10

# 3. base64 whitespace — z.string().base64() rejects whitespace
rg "\.base64\(\)" --type ts | grep -v node_modules | head -10
```

### ★ TypeScript 7.0 STABLE — SHIPPED JULY 8, 2026 (Missing from All Skill Docs)

**TypeScript 7.0 = Go-based compiler = 8–12x faster type checking.** This is the most impactful TypeScript release in history, but it has been **completely missing from all skill docs**. The skill tracked `typescript@next` dev builds but never documented the stable release.

**What changed**: The entire TypeScript compiler was rewritten in Go. Language features are the same as 6.x. The API for programmatic use is stable in 7.1.

**Setup recipe — add to all Next.js projects**:
```bash
...
# Next.js 16.3+ automatically uses TS7 for next build type checking
# No additional config needed

# For tsconfig.json — verify these settings for TS7 compatibility:
# "target": "ES2017" or higher (TS7 requires ES2017+)
# "module": "commonjs" | "esnext" | "nodenext" — all fine
# "moduleResolution": "bundler" — recommended for Next.js
```

**Setup check for existing projects**:
```bash
# Verify no deprecated tsconfig options
npx tsc --showConfig | grep -i "target\|module"

# TS7 dropped support for some very old target values
# If you see errors after upgrade, check:
