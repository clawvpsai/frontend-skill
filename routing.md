...
**`basePath` fallback parameter parsing fixed** (16.3.0-preview.8, [PR #95966](https://github.com/vercel/next.js/pull/95966), merged 2026-07-22T10:38:30Z):

In Next.js 16 with `experimental.basePath` configured, dynamic route parameters could shift when the server's response URL included a `basePath` prefix but the client-side App Router tree did not account for it — causing `params` in layout/page components to contain incorrect values after navigation. The root cause was in `packages/next/src/client/route-params.ts`: the fallback-parameter reconstruction logic used `getRenderedPathname()` which returned the full URL including `basePath`, but the App Router tree was already stripping `basePath` from its route matching. The fix strips `basePath` from the response URL pathnames before reconstructing fallback route parameters on the client.

A new E2E test suite `test/e2e/app-dir/basepath-fallback-params/` covers both direct hydration and client navigation with Cache Components enabled.

**Who is affected:** apps that use `basePath` in `next.config.ts` AND have dynamic route segments (`[id]`, `[slug]`, etc.) in the App Router, AND where the server redirect or response URL includes the `basePath` prefix.

**Action:** upgrade to `next@canary@94+` (expected 2026-07-22T23:00Z) or `next@preview@8+` (`npm install next@preview`). No config changes required.

**Source:** [PR #95966 — `Fix basePath fallback parameter parsing`](https://github.com/vercel/next.js/pull/95966) · merged 2026-07-22T10:38:30Z · ships in **`next@preview@8`** + canary.94.
...
## 16.3.1-canary.12-ahead — Fix Optimistic Routing Bugs Leading to Repeated Prefetch Loops (PR #97128) + 3-PR Legacy PPR Refactor (PR #96753 / #96827 / #96868 — Cache Components Now the Internal PPR Signal) + Turbopack CJS Scope-Hoisting Flag (PR #95826) + Turbopack import.meta.glob Module Type Arg (PR #96991) + Turbopack CJS Tree-Shaking Bail (PR #97130) — Routing-Lens (August 10–11, 2026)

The single most material Next.js routing-relevant change in the 6h window since v1.5.48: **PR #97128 — Fix Optimistic routing bugs leading to repeated prefetch loops** ([Janka Uryga](https://github.com/lubieowoce), merged 2026-08-11T12:44:48Z, 26 files changed, base `canary`; fixes #97135). **Next.js canary-branch is now 15 commits ahead of `16.3.1-canary.11`** (verified at this cron's check via `GET /repos/vercel/next.js/compare/v16.3.1-canary.11...canary` returning `ahead_by: 15`); the `v16.3.1-canary.12` version-tag was published at 2026-08-11T17:23:13Z (literally 39 min before this cron's 18:02Z start); npm `dist-tag.canary` still resolves to `16.3.1-canary.11` — canary.12 npm-publish expected ~2026-08-12T07:23Z ± a few hours on the 24h cadence. The 15 canary-branch-ahead commits decompose to **6 MATERIAL + 8 non-material + 1 version-tag** (the version-tag is the 15th commit, which is `40503aa` per `GET /repos/vercel/next.js/git/refs/tags/v16.3.1-canary.12`). The 6 MATERIAL commits: **(1) PR #97128** (optimistic routing prefetch loops — see headline below); **(2) PR #96753** (`d1123c9`) — `Make legacy PPR paths explicit` (renames the remaining legacy PPR render capability / prerender store / component / tracking helper from their old generic names to their `Legacy*` equivalents so Cache Components becomes the only path that toggles partial prerendering); **(3) PR #96827** (`d18e789`) — `Use Cache Components as the internal PPR signal` (closes the migration: `experimental.ppr` is already deprecated and configuring it with a truthy value already errors — this PR makes enabling Cache Components the sole path for partial prerendering, so the runtime signal for "this is a PPR render" comes from Cache Components alone); **(4) PR #96868** (`268ae98`) — `Remove legacy PPR code paths` (deletes the now-renamed legacy PPR code paths so only the Cache-Components-driven path remains — all three PRs land together as the v1.5.45/v1.5.46 forward-looking "3-PR legacy PPR refactor" prediction came true); **(5) PR #95826** (`5327653`) — `[turbopack] Do the CJS analysis needed for scope hoisting` (adds the new `turbopackCjsScopeHoisting` flag plumbed through `next.config.ts` + the CJS-analysis machinery in `turbopack/crates/turbopack-ecmascript/src/analyzer/graph/visitor.rs`); **(6) PR #97130** (`a5674d0`) — `[turbopack] Bail CJS tree-shaking on \`var x = module.exports = {}\`` (5 files changed; fixes the `@mixmark-io/domino` lib/LinkedList.js self-referential `var LinkedList = module.exports = { ... }; LinkedList.insertBefore(...)` pattern that PR #97018's canary.11 CJS-tree-shaking revert was needed for — *companion fix* that lets you opt-in to CJS tree shaking on canary.12+ without hitting the property-elision crash; verified with `var utils = require('./utils'); var LinkedList = module.exports = { valid: ..., insertBefore: ..., ... }` — the local-alias `LinkedList` was being silently dropped, leaving `LinkedList.insertBefore(...)` calls unresolvable; this PR teaches the tree-shaker to bail on the self-referential `var x = module.exports = {}` pattern). The 8 non-material commits: PR #97136 + PR #97137 (2 spelling fixes) + PR #97017 (test infra re-enable) + PR #97143 (docs: align Cache Components authentication guide title) + PR #96991 (`7b848a4`) — `Turbopack: accept a module type argument for import.meta.glob` (the TypeScript overloads gain a module-type parameter — non-material for the routing lens but documented in setup.md-equivalent coverage); PR #96341 (docs: client-side fetching guide) + PR #97180 (docs: clarify client cache freshness after hydration) + PR #96145 (docs: server client and directives). **Routing-lens material PRs in detail:**

### Why PR #97128 matters — Fix Optimistic Routing Bugs Leading to Repeated Prefetch Loops

Closes issue #97135 (and the upstream report at [MarkBekooy/prefetching-request-waterfall-bug#1](https://github.com/MarkBekooy/prefetching-request-waterfall-bug/issues/1)). **The bug** — a Next.js proxy that rewrites every URL to inject a leading path segment (`/one/two` → `/en/one/two`, i.e. i18n with the default locale hidden from the URL) plus a fully dynamic target route like `/[locale]/[...pages]` leads to an **infinite prefetch loop that never resolves**. Originally reported as a **next-intl request waterfall** — every render fires another prefetch, which fires another, ad infinitum, holding the runtime hostage. **The root cause** — the optimistic routing implementation didn't check whether the prefetch response matched the expected result. If the tree mismatched, in some cases the prefetch task would fall into a loop, repeatedly attempting to fulfill the missing data. **The fix** — when this mismatch happens, we now record on the local route definition that a dynamic rewrite occurred, disabling further attempts to optimistically resolve the route. **This is the same strategy we were already using for normal navigation responses, now applied to the prefetch path.** A second, related bug fixed in the same PR: **parallel routes with conflicting dynamic params at the same level** (`@modal/[...catchAll]` next to `[username]`) cannot be distinguished using the current traversal algorithm, because it assumes that each segment is resolvable independently without inspecting the children or sibling branches. To work around this for now, when this scenario is detected, optimistic routing is disabled for the conflicting route. **A proper fix** — having the server send down the "sibling" dynamic route segments, similar to what we already do for static siblings — is deferred to a future PR. **Practical impact**:

| Deployment profile | Symptom pre-#97128 | Post-#97128 |
|---|---|---|
| **i18n apps with default locale hidden from URL** (`/about` → `/en/about` via middleware/rewrite + dynamic `/[locale]/[...pages]` catch-all) | Infinite prefetch loop; the runtime never resolves; CPU pegged; memory grows; eventually OOM or hangs | Loop breaks on first rewrite mismatch; local route def records the rewrite; further attempts disabled; resolves normally |
| **Parallel routes with `@modal/[...catchAll]` next to `[username]`** | Optimistic routing attempts to traverse independently; segments appear unresolvable; stale/incorrect prefetch results; visible UI glitches | When the conflict is detected, optimistic routing is disabled for the conflicting route; correct data fetched via the normal (non-optimistic) path |
| **Static locale-with-URL apps** (e.g. `/en/about`, `/de/about` — no hidden locale) | Not affected | Not affected (no proxy rewrite) |
| **Apps using `next-intl` with default locale in URL** (`/en/about`) | Not affected (matches the i18n in URL, not the hidden-locale case) | Not affected |
| **Apps without parallel routes + without middleware URL rewrites** | Not affected | Not affected |

**Why this matters at the routing layer** — the loop fires through the App Router's optimistic routing path (the segment-cache + Navigation API hydration interplay), which is the exact same machinery that PR #96252 (Back-during-hydration race fix, canary.4) and PR #96950 (Server Actions routing refactor, forward-looking for 16.4) deal with. The bug is **deployment-shape-specific** — only triggers when (a) you have URL rewriting via proxy/middleware that adds/removes path segments AND (b) your target routes are fully dynamic OR (c) you have parallel routes with conflicting dynamic params at the same level. **Mitigation until canary.12 ships**:

```js
// next.config.ts — if you can't bump to canary.12 yet
export default {
  experimental: {
    // The flag exists in canary.12 to allow opting back into CJS tree shaking
    // once you've audited your app for the domino pattern (PR #97130 fix)
    turbopackCjsScopeHoisting: true, // PR #95826 — opt-in to CJS scope hoisting
  },
};
```

### Why PR #96753 / #96827 / #96868 matters — Cache Components Now the Internal PPR Signal + Legacy PPR Code Paths Removed

The v1.5.45/v1.5.46 forward-looking "3-PR legacy PPR refactor" prediction came true: **PR #96753** renames the remaining legacy PPR render capability / prerender store / component / tracking helper from their old generic names to their `Legacy*` equivalents; **PR #96827** migrates `experimental.ppr` to error-on-truthy-value (already done in earlier cycles) AND makes Cache Components the sole internal signal for partial prerendering; **PR #96868** deletes the legacy code paths now that Cache Components is the only one left. **Practical impact**:

- **Anyone using `experimental.ppr: true` with `next@16.3.1-canary.12+`** — the option will continue to error at config-eval time (the deprecation has been in place since 16.3.0 STABLE). Migration path: switch to `experimental.cacheComponents: true` (the new public API). See `server-components.md` for the full Cache Components setup recipe.
- **Anyone using Cache Components today** — no behavior change; the internal signal source moved but the runtime semantics are identical. This is a pure internal refactor + deprecation enforcement.
- **Anyone using neither** — no behavior change; PPR was off for them before and remains off.
- **Framework authors wrapping Next.js internals** — the `Legacy*` prefix is now uniform across render-capability / prerender-store / component / tracking-helper. If you imported any of these by name, search-and-replace to add the `Legacy` prefix.
- **Audit recipe** — `rg -n "experimental.ppr" next.config.ts next.config.js` (should return 0 lines for `canary.12+` users; the build will fail at config-eval time otherwise). `rg -n "Ppr|ppr" packages/next/src/server/` (will find the `Legacy*` renames in canary.12+).

### Why PR #95826 matters — Turbopack CJS Scope-Hoisting Flag (Opt-In for canary.12+)

Adds a new `turbopackCjsScopeHoisting` config flag plumbed through `next.config.ts` + the CJS-analysis machinery in `turbopack/crates/turbopack-ecmascript/src/analyzer/graph/visitor.rs`. **Practical impact** — opt-in to CJS scope-hoisting analysis to enable further tree-shaking of CommonJS modules on Turbopack. **Zero behavior change** if not opted-in. **Audit recipe** — `rg -n "turbopackCjsScopeHoisting" next.config.ts` (set to `true` after auditing your app's CJS deps for the `@mixmark-io/domino` pattern; canary.12+ only).

### Why PR #97130 matters — Turbopack CJS Tree-Shaking Bail (Complementary Fix for PR #97018's Revert)

The `var x = module.exports = {}` pattern (used by `@mixmark-io/domino`'s `lib/LinkedList.js` among others) was the canonical reason PR #97018 reverted CJS tree-shaking in canary.11. **PR #97130 teaches the CJS tree-shaker to bail on this pattern** — instead of incorrectly dropping the `LinkedList` local alias, the analysis now recognizes the self-referential assignment and skips tree-shaking for the affected module. **Practical impact** — combined with PR #95826's `turbopackCjsScopeHoisting` flag, this PR lets you opt back into CJS tree shaking on canary.12+ without hitting the property-elision crash that PR #97018 was reverting. **Audit recipe** — `rg -n "module\.exports\s*=" node_modules/@mixmark-io/domino/lib/LinkedList.js node_modules/cheerio/lib/ node_modules/jsdom/lib/` (find any module using the self-referential pattern; should be safe to opt-in after PR #97130).

### Common Mistakes — Routing-Edition Updates

The Common Mistakes section grows **2 new bullets**:
- **i18n apps with default locale hidden from URL (`/about` → `/en/about` via middleware/rewrite) + dynamic catch-all routes cause an infinite prefetch loop (16.3.0 STABLE + canary.0–11) — FIXED in `next@16.3.1-canary.12-ahead` by PR #97128** — the bug is the segment-cache optimistic routing falling into a retry loop when the rewrite-induced tree mismatches the expected result. If you're on `16.3.0` STABLE or any `16.3.1-canary.0–11` and seeing this loop with `next-intl` or a custom i18n middleware: bump to `next@>=16.3.1-canary.12` when it npm-publishes (~2026-08-12T07:23Z ± a few hours). **Workaround until canary.12 ships** — disable optimistic routing for the affected routes by setting `experimental.optimisticRouting: false` (if exposed on your current version) or by switching from a hidden-locale proxy rewrite to a static-prefix approach (`/en/about` literal URLs). See the migration recipe in next-intl docs.
- **Parallel routes with `@modal/[...catchAll]` next to `[username]` produce incorrect prefetch results (16.3.0 STABLE + canary.0–11) — FIXED in `next@16.3.1-canary.12-ahead` by PR #97128** — the segment-cache traversal can't distinguish conflicting dynamic params at the same level. Post-#97128, optimistic routing is disabled for the conflicting route. If you can't bump yet: avoid parallel routes that share dynamic-segment levels with conflicting shapes, or rename one side to use a static prefix.

### Sources

- [PR #97128 — Fix: Optimistic routing bugs leading to repeated prefetch loops](https://github.com/vercel/next.js/pull/97128) — by Janka Uryga (lubieowoce), merged 2026-08-11T12:44:48Z, 26 files changed, base `canary`. **THE HEADLINE of canary.12-ahead for the routing lens.** Closes #97135 + the [MarkBekooy/prefetching-request-waterfall-bug#1](https://github.com/MarkBekooy/prefetching-request-waterfall-bug/issues/1) next-intl request waterfall report.
- [Issue #97135 — Optimistic routing causes repeated prefetch loops](https://github.com/vercel/next.js/issues/97135) — the canonical bug report closed by PR #97128.
- [MarkBekooy/prefetching-request-waterfall-bug#1](https://github.com/MarkBekooy/prefetching-request-waterfall-bug/issues/1) — the upstream next-intl request-waterfall reproduction.
- [PR #96753 — Make legacy PPR paths explicit](https://github.com/vercel/next.js/pull/96753) — by Janka Uryga (lubieowoce), merged 2026-08-11T01:39:14Z, base `canary`. Part of the 3-PR legacy PPR refactor.
- [PR #96827 — Use Cache Components as the internal PPR signal](https://github.com/vercel/next.js/pull/96827) — by Janka Uryga (lubieowoce), merged 2026-08-11T01:39:14Z, base `canary`. The substantive change in the refactor.
- [PR #96868 — Remove legacy PPR code paths](https://github.com/vercel/next.js/pull/96868) — by Janka Uryga (lubieowoce), merged 2026-08-11T01:39:14Z, base `canary`. The cleanup in the refactor.
- [PR #95826 — [turbopack] Do the CJS analysis needed for scope hoisting](https://github.com/vercel/next.js/pull/95826) — by Tobias Koppers (sokra), merged 2026-08-11T07:06:04Z, base `canary`. Adds `turbopackCjsScopeHoisting` flag.
- [PR #97130 — [turbopack] Bail CJS tree-shaking on `var x = module.exports = {}`](https://github.com/vercel/next.js/pull/97130) — merged 2026-08-11T16:41:27Z, 5 files, base `canary`. Companion fix for PR #97018's revert.
- [PR #96991 — Turbopack: accept a module type argument for `import.meta.glob`](https://github.com/vercel/next.js/pull/96991) — merged 2026-08-11T10:05:56Z. TypeScript overloads gain a module-type parameter.
...
- [Next.js `v16.3.1-canary.12` GitHub release tag](https://github.com/vercel/next.js/releases/tag/v16.3.1-canary.12) — published 2026-08-11T17:23:13Z; npm `dist-tag.canary` still resolves to `16.3.1-canary.11`; npm-publish expected ~2026-08-12T07:23Z ± a few hours on the 24h cadence.
...
- Cross-reference: `server-components.md` → `## Next.js 16.3.x Cache Components — 3-PR Legacy PPR Refactor (PR #96753 / #96827 / #96868 — canary.12-ahead) + PR #95826 Turbopack CJS Scope-Hoisting Flag` for the Cache Components lens on the same PRs.
...
- **`headers()` returns stale pre-mutation values in Proxy after `request.headers.set(...)` (16.3.0 STABLE + canary.0–13) — FIXED in `next@16.3.1-canary.14` by PR #97166** — Hendrik Liebau, merged 2026-08-12T11:36:12Z, 8 files / +382/-23, base `canary`. The bug: PR #94703 + #95116 (the dev-request-id-header cleanup) accidentally turned the sealed `headers()` view into a frozen snapshot instead of a live read-through view. Two APIs describing the same request disagreed: `request.headers.get(...)` returned the new value, but `headers()` returned the pre-mutation value. Symptom: tracing/feature-flag/tenant-id headers injected by Proxy don't propagate to RSCs that read via `headers()`. Fix: bump to `next@>=16.3.1-canary.14` (shipped at 2026-08-12T13:25:30Z).: read directly via `request.headers.get(name)` instead of relying on `headers()` to reflect the mutation. See the new `### Why PR #97166 matters` subsection above for full walkthrough + audit recipe.
- **Turbopack emits `crossorigin=""` on `<script>` tags when `crossOrigin` is unset, breaking cross-origin `assetPrefix` CDNs (16.3.0 STABLE + canary.0–13) — FIXED in `next@16.3.1-canary.14` by PR #97164** — Tim Neutkens, merged 2026-08-12T07:19:23Z, 7 files / +164/-31, base `canary`. The bug: Turbopack's client reference manifest changed from an optional cross-origin value to a `CrossOrigin` enum in PR #92070, and Serde serialized `CrossOrigin::None` as the string `"none"` instead of omitting it. Symptom: preinited chunks carry `crossorigin=""`; on real CDNs whose cache key doesn't include the `Origin` header, the no-cors responses poison the CORS-mode loads; chunks load intermittently. Closes issue #96831 (one of the 3 NEW open issues on 16.3.0 STABLE that v1.5.33 documented in `deployment.md`). Fix: bump to `next@>=16.3.1-canary.14` (shipped at 2026-08-12T13:25:30Z). **Workaround until then**: configure `Access-Control-Allow-Origin` on the CDN, switch to Webpack, use `output: 'export'`, or roll back to `next@16.2.12`. See the new `### Why PR #97164 matters` subsection above for full walkthrough + audit recipe.

### Forward-Looking

- **Next.js `v16.3.1-canary.14` SHIPPED at 2026-08-12T13:25:30Z** — landed ~4h38min before the v1.5.51 prediction of `~18:23Z ± a few hours`; consistent with the accelerated 1.2 commits/hour cadence observed in the 5h49min window. Bundled: PR #97208 + PR #95439 + PR #97215 + PR #97181 (all 4 from v1.5.50) + PR #97164 (crossOrigin fix) + PR #97166 (headers() live view restore) + PR #93529 + PR #97191 + PR #97187 + PR #97163 + PR #97190 + the version-tag commit. Exactly as predicted by v1.5.51.
...
- [PR #97164 — `[turbopack]` Fix unset `crossOrigin` in Turbopack manifests](https://github.com/vercel/next.js/pull/97164) — by Tim Neutkens (timneutkens), merged 2026-08-12T07:19:23Z, 7 files / +164/-31, base `canary`. The second material commit in canary.14 SHIPPED for the routing lens.
...
- Cross-reference: `server-components.md` → `## headers() Restored to Live View of Incoming Request (PR #97166) + Turbopack `crossOrigin` Manifest Fix (PR #97164) — SHIPPED in 16.3.1-canary.14 (August 12, 2026)` for the Server Components lens on the same PRs.
