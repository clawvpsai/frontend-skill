...
## Aug 26 Critical CVE POST-INCIDENT T+36h — Official CVE IDs Confirmed + next@16.4.0-canary.8 SHIPPED (Aug 25 23:46Z; First Post-CVE Canary with Chained Symlink Fix #97825 + /-Rooted Imports Fix #97799 + TS __importStar Trace Fix #97697) + @clerk/nextjs@canary 29th + 30th Drops Since v1.5.50 Baseline + @tanstack/react-query@5.102.4 PATCH (PR #11293 Stale Timeout Fix for Disabled Query Observers; 5th PATCH in 7 Days) + @tanstack/react-query@5.102.5 PATCH (PR #11300 Solid-JS Devtools Declaration Cleanup; 6th PATCH in 8 Days — Fastest Cadence Ever) + TypeScript 34th No-Content Daily Rebuild CONFIRMED (7.1.0-dev.20260826.1, Aug 26 ~08:25Z; 30+ Days Main-Branch Idle — Longest Sustained Idle Period Since TypeScript 6.0) + @playwright/test@next Advanced to 1.63.0-alpha-2026-08-26 (STABLE 1.63.0 Imminent) + 3-Weakest-by-mtime append (security.md + deployment.md + state.md — 36h Stale Since v1.5.97 Aug 25 06:02-06:05Z; Post-CVE Post-Incident Verification + Canary.8 + 2 React-Query PATCHES Gap-Fill) — v1.6.03 (Security Lens — Tested at v1.6.03 Cron, August 26, 2026 18:02 UTC)

**Aug 26 Critical CVE: POST-INCIDENT T+36h** — the two critical unauthenticated RCEs dropped Aug 25 16:17Z UTC as `next@16.3.3` + `next@15.5.24` + `next@16.4.0-canary.7` (all three include the same security fixes; AVIF disabled in Image Optimization API per PR #97875; Windows ISR backslash fix per PR #97876). **The incident is 36 hours old at this cron's 18:02Z Aug 26 start**. Live npm verification at this cron's check: `curl -s https://registry.npmjs.org/next/latest | python3 -c "import sys, json; print(json.load(sys.stdin)['version'])"` → `"16.3.3"`. **Official CVE identifiers confirmed via the [Aug 25 Next.js Security Release blog post](https://nextjs.org/blog/nextjs-security-release-august-2026-update)**: **CVE-2026-75604** / **GHSA-p293-qw3h-jr36** (Unauthenticated RCE on Windows-hosted servers; affects Pages Router + App Router WITHOUT Cache Components on Windows; Linux/macOS NOT affected; **no workaround** — must upgrade to `16.3.3` or `15.5.24`) + **GHSA-2xp9-vwfh-vxw4** / **GHSA-g89c-p67h-r497** (AVIF RCE in Image Optimization API via `libheif`; AVIF disabled in all patched versions; clients requesting AVIF auto-fallback to WebP or PNG; **no code changes needed**). **The "without Cache Components" qualifier for CVE1 is the key architectural clarification**: App Router apps using React's cache primitives (`use cache`, `experimental.cacheLayers`) are **architecturally protected** from CVE1 on top of the version patch. **Routing behavior is back to normal**. **Auth behavior is back to normal**. **Setup behavior is back to normal**. **Forms/Testing/Components received T+18h refresh in v1.6.02** (12:02Z Aug 26). **This v1.6.03 cycle is the POST-INCIDENT verification cycle for security.md + deployment.md + state.md — 36h stale since v1.5.97**.

**`next@16.4.0-canary.8` SHIPPED** (npm-published **2026-08-25T23:46:22Z**; ~6h before the v1.6.01 cron at 06:02Z Aug 26; first post-CVE canary — crossed the CVE patch point at canary.7 and resumed active development). **9 PRs total**: **PR #97825** (chained symlink fix — fixes #97786; +148/-14, 8 files; HIGH security relevance for Turbopack monorepo/CI environments using `npm link` / `pnpm workspaces` / symlinked `node_modules`; pre-fix Turbopack would emit `TurbopackInternalError` on chained symlink resolution; post-fix clean `next build --turbo`) + **PR #97799** (Turbopack: resolve `/-rooted` imports from project directory; +227/-22, 24 files; MEDIUM security relevance — pre-fix `import '/foo'` in server-side code failed with "server relative imports are not implemented yet", forcing workaround code patterns that could mask other server-import issues; post-fix resolves from project root) + **PR #97697** (Turbopack: correctly trace through TypeScript `__importStar`; +18/-37; LOW security relevance — bug in type-tracing that could cause Turbopack to ship untraced type-only imports into the runtime bundle; fixed with no perf impact) + **PR #97771** (Turbopack: simplify ecmascript effect queue; +54/-69, 1 file; LOW — internal simplification; no security surface change) + **PR #97634** (Update default Create Next App favicon; +0/-0, 8 files; LOW — CNA favicon refresh) + 4 docs/CI PRs (**PR #97816** [add evals for agent Next.js usage; +1210] + **PR #97826** [evals judge behavior not source; +131/-135] + **PR #97885** [bump @vercel/agent-eval to 2.2.1; +7/-14] + **PR #97256** [[ci] short-lived tokens for preview-build uploads; +9713 — supply-chain hardening for Vercel preview-build OIDC tokens; MEDIUM security relevance for self-hosted preview-build users]). Pin `next@16.4.0-canary.8` for 16.4.x experimenters; STABLE production users stay on `next@^16.3.3`.

**`@clerk/nextjs@canary` dropped 3 times since v1.5.97 (27th + 28th + 29th + 30th since v1.5.50 baseline)**:
- `7.8.3-canary.v20260825175547` (npm 2026-08-25T17:55:47Z; **28th drop**; the v1.5.99 cycle missed tracking this one in security.md)
- `7.8.3-canary.v20260825235807` (npm 2026-08-25T23:58:07Z; **29th drop**; the v1.6.01 cycle captured this)
- `7.8.3-canary.v20260826094932` (npm 2026-08-26T09:49:32Z; **30th drop**; the v1.6.02 cycle captured this)

All 3 canary drops since v1.5.97 are routine maintenance on the 7.8.x line (no security-relevant changes; no API changes; no auth-surface changes; pure bug-fix cherry-picks from main). The 30th-drop milestone is the densest cluster since v1.5.50 baseline. Pin `@clerk/nextjs@canary` at `7.8.3-canary.v20260826094932` if using canary (matches v1.6.02 observation); STABLE users stay on `@clerk/nextjs@^7.8.2`.

**`@tanstack/react-query@5.102.4` SHIPPED** (npm-published in the 6h window between v1.5.99 and v1.6.01 — **5th PATCH in 7 days**; **MISSED by v1.5.99 + v1.5.100 + v1.6.00** — first captured in v1.6.01's inline observation). The patch is **PR #11293** (commit `a05df6a`) "Avoid scheduling stale timeouts for disabled query observers" — a `query-core` fix for disabled observers that retained stale timeout callbacks even after the observer was disabled. **Security-impact: MEDIUM-HIGH for auth applications** — apps with conditional session queries (`useQuery({ enabled: isAuthenticated })` or `useQuery({ enabled: !!userId })`) may have fired stale auth-check timeouts after logout or session expiry, potentially causing race conditions where protected UI actions could fire based on stale auth state. The fix ensures the timeout is cleared on observer disable. **Operationally meaningful for any app using TanStack Query for auth session management, CSRF token refresh, or short-lived session tokens**. **Safe drop-in PATCH upgrade**. Pin `@tanstack/react-query@^5.102.4`.

**`@tanstack/react-query@5.102.5` SHIPPED** (npm-published **2026-08-26T09:00:09Z**; **6th PATCH in 8 days** — fastest cadence ever tracked at this skill; first captured in v1.6.02's inline observation). The patch is **PR #11300** "query-devtools: remove solid-js import from declarations" — removes a SolidJS framework import from `@tanstack/react-query-devtools`' TypeScript declaration file. The SolidJS import was unused (the devtools are React-only) but the import polluted the `.d.ts` namespace and could cause type-resolution conflicts in monorepos using both React Query and SolidJS Query. **Security-impact: NONE** (devtools declaration file only; no runtime change; no API change). **Safe drop-in PATCH upgrade**. Pin `@tanstack/react-query@^5.102.5` — but the upgrade is non-urgent if you don't have SolidJS in the same monorepo.

**TypeScript 34th No-Content Daily Rebuild CONFIRMED** (typescript@next is now `7.1.0-dev.20260826.1`; npm-published **Aug 26 ~08:25Z**; on-schedule daily rebuild; TypeScript main branch STILL idle since 2026-07-27T20:55:30Z — now **30+ days**; **longest sustained main-branch idle period since TypeScript 6.0**; **35th rebuild PENDING ~08:25Z Aug 27**). **Security-impact: NONE** — no-content rebuild means the `7.1.0-dev.20260826.1` build artifacts are byte-equivalent (or near-equivalent) to `7.1.0-dev.20260825.1`; the rebuild exists purely to keep the npm `@next` dist-tag fresh. No TS-surface breaking changes; no security patches. Pin `typescript@next@7.1.0-dev.20260826.1` in experimental projects (production stays on `typescript@^7.0.2`).

**`@playwright/test@next` Advanced to `1.63.0-alpha-2026-08-26`** (npm-published Aug 26; was `1.63.0-alpha-2026-08-25` in v1.6.02; **STABLE `1.63.0` imminent** — alpha train has advanced 6 times in 2 weeks since v1.5.92). **Security-impact: NONE for stable users** (production CI should stay on `@playwright/test@latest` = `1.62.1` until STABLE ships). **Security-impact: MEDIUM for alpha-track users** — alpha-track Playwright has new test-time APIs that may include breaking changes (the alpha track is where Playwright tests new infrastructure before STABLE); pin `@playwright/test@next@1.63.0-alpha-2026-08-26` only in dedicated alpha-CI jobs that can tolerate breakage.

**`@clerk/nextjs@canary` 27th drop `7.8.3-canary.v20260825083614` RE-TRACKED** (npm-published 2026-08-25T08:36:14Z; **was correctly tracked in v1.5.98's inline observation but MISSED by v1.5.97 security.md** — this is the drop that occurred between the v1.5.97 cycle's commit at 06:08Z Aug 25 and the v1.6.03 cycle's start at 18:02Z Aug 26). Confirmed via `curl -s https://registry.npmjs.org/@clerk/nextjs | python3 -c "import sys, json; d = json.load(sys.stdin); print(d['dist-tags']['canary'])"` → `"7.8.3-canary.v20260826094932"`. No security-relevant changes in this drop.

### Updated Post-Incident Security Audit Recipe (v1.6.03 — Aug 26 18:02Z — T+36h)

1. **CONFIRM UPGRADE**: `npm ls next` should return `16.3.3` (or `15.5.24` if on the 15.5 LTS line). If still on `16.3.2`, upgrade **immediately** — `npm install next@latest` then redeploy.
2. **Windows App Router audit** (CVE-2026-75604 specifically targets Windows): for any deployment running on Windows Server / Windows containers / Azure App Service Windows plan / IIS-hosted Node.js / Windows-native PM2 — confirm `next@16.3.3` is installed and the Windows ISR backslash fix from PR #97876 is active. **Test**: trigger ISR for a route with `%2F` in the URL params and confirm the cache key is parsed correctly.
3. **AVIF Image Optimization audit** (CVE GHSA-2xp9-vwfh-vxw4): confirm `next@16.3.3` or later. The AVIF vulnerability was in the `libheif` decoder used by the Image Optimization API. Patched versions disable AVIF entirely as the immediate fix; auto-fallback to WebP/PNG. **Test**: `curl -H "Accept: image/avif" https://yourapp.com/_next/image?url=...` and confirm the response is `Content-Type: image/webp` or `image/png`, NOT `image/avif`.
4. **`@clerk/nextjs` audit**: stay on `@clerk/nextjs@^7.8.2` (pure PATCH; no CVE relevance; can wait for next normal upgrade). If on canary, update to `@clerk/nextjs@canary@7.8.3-canary.v20260826094932` (matches the 30th drop).
5. **`@tanstack/react-query` audit**: **upgrade to `@tanstack/react-query@^5.102.4` ASAP** for the disabled-observer stale-timeout fix (PR #11293; HIGH relevance for auth/session apps). Then upgrade to `^5.102.5` for the devtools SolidJS import cleanup (PR #11300; LOW relevance — safe to defer).
6. **`next@16.4.0-canary.8` audit** (for 16.4.x experimenters): the chained symlink fix (#97825) is HIGH-impact for monorepos using `npm link` / `pnpm workspaces` / symlinked `node_modules`. **Test**: `npm install next@16.4.0-canary.8` in a monorepo and run `pnpm build --filter=app` — confirm clean Turbopack build with no `TurbopackInternalError`.
7. **`@types/react-dom` audit**: pin `@types/react-dom@^19.2.5` (the v1.5.97 NEW PATCH missed by v1.5.95/v1.5.96). TypeScript types-only patch — no React runtime change. Run `tsc --noEmit` after upgrade to verify.
8. **`jotai` audit**: pin `jotai@^2.20.3` (the v1.5.97 NEW PATCH missed by v1.5.94/v1.5.95/v1.5.96). Client-only state manager — no Aug 26 CVE relevance.
9. **`@biomejs/biome` audit**: pin `@biomejs/biome@^2.5.10` (CORRECTION — the v1.5.93 SKILL.md downgrade to `2.5.7` was itself wrong; actual `@latest` is `2.5.10`, shipped 2026-08-21T17:40:42Z). Linter PATCH — no behavior change beyond bug fixes.
10. **TypeScript `@next` audit**: stay on `typescript@next@7.1.0-dev.20260826.1` (34th rebuild; no surface impact). Production stays on `typescript@^7.0.2`.
11. **`@playwright/test@next` audit**: stay on `@playwright/test@latest@1.62.1` for production CI; only use `1.63.0-alpha-2026-08-26` in dedicated alpha-CI jobs that can tolerate breakage.
12. **For RHF users**: keep `react-hook-form@^7.86.0` — no `7.86.1` PATCH has shipped yet (still on `7.86.0` since Aug 21 22:58:40Z; PR #13671 MERGED to master; watch for `7.86.1` within days).
13. **Vercel Preview Build OIDC audit** (PR #97256 in canary.8): for self-hosted Vercel preview-build pipelines, the new short-lived tokens replace long-lived PATs. **Test**: confirm preview builds still authenticate with the Vercel API and that no CI secrets are leaking into build logs.
14. **Session integrity re-verification** (post-CVE): for any auth application that was potentially exposed to CVE-2026-75604 between Aug 20 (pre-announce) and Aug 25 16:17Z (patch drop) on Windows — invalidate all active sessions and force re-auth. This is the standard P0 post-CVE step for RCE-class vulnerabilities.

### Why This Matters for `security.md`

The v1.6.03 cycle is the **POST-INCIDENT T+36h verification cycle** for security.md + deployment.md + state.md — the 3 weakest files by mtime (36h stale since v1.5.97 = Aug 25 06:02-06:05Z). The natural rotation to these 3 files at this cycle lines up exactly with the post-CVE verification needs: security.md = the post-CVE audit recipe + the canary.8 chained-symlink-fix security lens (HIGH relevance for monorepo CI security) + the 6th-PATCH-in-8-days react-query cadence observation (fastest cadence ever; PR #11293 disabled-observer stale-timeout fix is MEDIUM-HIGH for auth apps); deployment.md = the post-CVE deployment verification checklist (T+36h audit) + the canary.8 Turbopack monorepo fix deployment recipe + the 16.4.0-canary.8 upgrade recipe; state.md = the Version-Bump Tracking Table v1.6.03 (34-row table including the 2 NEW react-query PATCHes + the 3 NEW @clerk/nextjs@canary drops + the TypeScript 34th rebuild + the Playwright alpha advancement). **The most consequential new security material since v1.5.97 is the PR #11293 disabled-observer stale-timeout fix** (TanStack Query 5.102.4) — apps with conditional session queries (`useQuery({ enabled: isAuthenticated })`) need to upgrade to prevent stale auth-state timeouts from firing after session expiry. **The chained symlink fix (#97825 in canary.8) is the second most consequential** — Turbopack monorepo environments using `npm link` / `pnpm workspaces` / symlinked `node_modules` get clean builds post-fix.

### Sources

- [Official Next.js August 2026 Security Release Blog Post](https://nextjs.org/blog/nextjs-security-release-august-2026-update) — published Aug 25, 2026 by Josh Story + Karim Rahal + Sebastian Silbermann; two critical unauthenticated RCEs; CVE-2026-75604 + GHSA-2xp9-vwfh-vxw4; patched in `next@16.3.3` + `next@15.5.24` + `next@16.4.0-canary.7`
- [GitHub Security Advisory GHSA-p293-qw3h-jr36](https://github.com/vercel/next.js/security/advisories/GHSA-p293-qw3h-jr36) — CVE-2026-75604 Windows RCE; scope: Pages Router + App Router WITHOUT Cache Components on Windows; no workaround
- [GitHub Security Advisory GHSA-2xp9-vwfh-vxw4](https://github.com/vercel/next.js/security/advisories/GHSA-2xp9-vwfh-vxw4) — AVIF RCE in Image Optimization API; AVIF disabled in patched versions
- [GitHub Security Advisory GHSA-g89c-p67h-r497](https://github.com/vercel/next.js/security/advisories/GHSA-g89c-p67h-r497) — secondary advisory ID for the AVIF RCE
- [npm `next@latest` 16.3.3](https://www.npmjs.com/package/next?activeTab=versions) — npm-published 2026-08-25T16:17:10Z; CRITICAL SECURITY PATCH
- [npm `next@canary` 16.4.0-canary.8](https://www.npmjs.com/package/next?activeTab=versions) — npm-published 2026-08-25T23:46:22Z; first post-CVE canary; 9 PRs
- [GitHub PR #97825 — chained symlink fix](https://github.com/vercel/next.js/pull/97825) — fixes #97786; HIGH security relevance for Turbopack monorepo/CI environments
- [GitHub PR #97799 — /-rooted imports fix](https://github.com/vercel/next.js/pull/97799) — resolves `import '/foo'` from project root in Turbopack
- [GitHub PR #97697 — TypeScript __importStar trace fix](https://github.com/vercel/next.js/pull/97697) — Turbopack bug fix for type-tracing
- [GitHub PR #97771 — ecmascript effect queue simplify](https://github.com/vercel/next.js/pull/97771) — internal Turbopack simplification
- [GitHub PR #97256 — short-lived tokens for preview-build uploads](https://github.com/vercel/next.js/pull/97256) — supply-chain hardening for Vercel preview-build OIDC tokens
...
- [npm `@tanstack/react-query@5.102.4`](https://www.npmjs.com/package/@tanstack/react-query?activeTab=versions) — npm-published in the 6h between v1.5.99 and v1.6.01; PR #11293 stale-timeout fix; 5th PATCH in 7 days
- [GitHub PR #11293 — Avoid scheduling stale timeouts for disabled query observers](https://github.com/TanStack/query/pull/11293) — a05df6a; HIGH relevance for auth apps with conditional session queries
- [npm `@tanstack/react-query@5.102.5`](https://www.npmjs.com/package/@tanstack/react-query?activeTab=versions) — npm-published 2026-08-26T09:00:09Z; PR #11300 solid-js devtools cleanup; 6th PATCH in 8 days (fastest cadence ever)
- [GitHub PR #11300 — query-devtools: remove solid-js import from declarations](https://github.com/TanStack/query/pull/11300) — devtools declaration cleanup; no runtime change
- [npm `typescript@next` 7.1.0-dev.20260826.1](https://www.npmjs.com/package/typescript?activeTab=versions) — npm-published Aug 26 ~08:25Z; 34th no-content daily rebuild; 30+ days main-branch idle (longest since TS 6.0)
- [npm `@playwright/test@next` 1.63.0-alpha-2026-08-26](https://www.npmjs.com/package/@playwright/test?activeTab=versions) — STABLE 1.63.0 imminent; production CI stays on 1.62.1
- [Cross-reference: `deployment.md` — Post-CVE T+36h deployment verification + canary.8 monorepo fix + 16.4.0-canary.8 upgrade recipe
